...
class Program
{
    ...
    {
        // Caminho do diretório onde os PDFs estão localizados
        string sourceDirectory = @"\\dc\F\Lidersis\ARQUIVO ATIVO\Auditorias\JVS\Declaracao_Conformidade";
        ...
        foreach (var pdfFile in pdfFiles)
        {
            try
            {
                ...
                if (!string.IsNullOrEmpty(candidateName))
                {
                    ...
                    string newFileName = $"{candidateName} - DECLARAÇÃO DE CONFORMIDADE.pdf";
                    string newFilePath = Path.Combine(sourceDirectory, newFileName);
